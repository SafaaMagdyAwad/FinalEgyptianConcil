                ...
                <tbody>
                    @foreach ($topics as $topic)
                        <tr>
                            ...
                            <td><a class="text-decoration-none text-dark" href="{{ route('topic.show',$topic) }}">{{ $topic->title }}</a>
                            </td>
                            ...
                            <td>
                                <form method="POST" action="{{ route('topic.destroy',$topic) }}">
                                    @csrf
                                    @method('delete')
                                    <button style="background-color: transparent; border: none; color: inherit;" type="submit"  onclick = "return confirm('Are you sure you want to delete this item?')">
